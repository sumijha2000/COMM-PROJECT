          ...
          public async Task<responseData> GetUserDataByContact(requestData rData)
{
    ...
    try
    {
        ...
        {
            var contact = rData.addInfo["EMAIL"].ToString(); // Ensure CONTACT value is retrieved correctly
            var query = @"SELECT * FROM mydb.uploader WHERE EMAIL=@EMAIL";
            MySqlParameter[] myParam = new MySqlParameter[]
            {
                new MySqlParameter("@EMAIL", contact)
            };

            var userData = ds.executeSQL(query, myParam).FirstOrDefault();
            if (userData != null)
            {
                resData.rData["userData"] = userData;
                resData.rData["rMessage"] = "Retrieved data successfully.";
            }
            else
            {
                resData.rData["rMessage"] = "No data found for the given contact.";
            }
        }
        else
        {
            resData.rData["rMessage"] = "CONTACT parameter not provided.";
        }
    }
    catch (Exception ex)
    {
